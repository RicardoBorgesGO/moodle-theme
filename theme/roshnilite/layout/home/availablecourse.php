...
foreach ($course as $key => $coursevalue) {
    ...
    if (!empty($coursevalue->summary)) {
        $coursedetailsarray[$key]["summary"] = $coursevalue->summary;
    }

    $teacher = $DB->get_records_sql("
        SELECT U.*
            FROM {course} CS, {context} CT, {role_assignments} RA, {user} U
            WHERE CS.ID = CT.INSTANCEID
                AND CT.ID = RA.CONTEXTID
                AND RA.USERID = U.ID
                AND RA.ROLEID = 3
                AND CT.CONTEXTLEVEL = 50
                AND CS.ID = ?", array($coursevalue->id));
    
    if ($teacher) {
        foreach ($teacher as $key2 => $teacherValue) {
            $coursedetailsarray[$key]["teacher"] = $teacherValue;//->firstname . " " . $teacherValue->lastname;
        }
    }
...
?>                  <a href="<?php echo $avlcoursearrayvalue["courseid"]; ?>">
...
?> 
                    <div class="data-teacher">
                        <?php 
                            if ($avlcoursearrayvalue["teacher"]) {
                                echo $OUTPUT->user_picture($avlcoursearrayvalue["teacher"]);
                        ?>
                        <?php
                                echo $avlcoursearrayvalue["teacher"]->firstname . " " . $avlcoursearrayvalue["teacher"]->lastname;
                            }
                        ?>
                    </div>
                    <div class="ribbon">
                    ...
                    <div class="course">
                        <div class="data-course">
                            <h4>
                                <a href="<?php echo $avlcoursearrayvalue["courseid"]; ?>">
                                    <?php echo $avlcoursearrayvalue["coursename"]; ?>
                                </a>
                            </h4>
                        </div>

                        <div class="data-summary">
                             <?php echo $avlcoursearrayvalue["summary"]; ?>
                        </div>
                    </div>
<?php
        }
